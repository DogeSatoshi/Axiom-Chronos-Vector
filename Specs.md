...
1. **Ingestion**: The user selects a supported asset, timeframe, and historical duration.
2. **Vectorization**: Received OHLCV data is converted into the local computational representation.
3. **Integrity Audit**: The Integrity Engine evaluates the dataset before strategy execution.
4. **Signal Generation**: The selected strategy produces its signal vector.
5. **Execution**: The execution engine applies the selected trading, fee, slippage, and risk assumptions to produce an equity curve and trade history.
6. **Intelligence Synthesis**: Quantitative metrics, benchmark analysis, and probability analysis are calculated.
7. **Session Cleanup**: Temporary market data is cleared according to the application's session lifecycle and privacy model.
...
For a concise public overview, see **[README.md](README.md)**.
